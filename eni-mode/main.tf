...
module "vpc" {
  ...
  private_subnets = ["10.2.32.0/19", "10.2.64.0/19", "10.2.96.0/19"]
...
resource "aws_subnet" "pods_subnets" {
  ...
  vpc_id            = module.vpc.vpc_id
  cidr_block        = cidrsubnet(element(local.secondary_cidr_blocks, 0), 2, count.index)
  ...
  tags = merge(
    ...
    {
      Name  = "${module.vpc.name}-pods-subnets-${local.azs[count.index]}",
      Usage = "pods"
    }
  )
}

resource "aws_route_table_association" "pods_subnets" {
  count = length(module.vpc.private_subnets[*])
